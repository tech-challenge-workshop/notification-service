 ...
 *
 * Unlike the Catalog and the Worker, this service declares one queue and is
 * currently its only declarer, so it has nothing to conflict with today. The
 * guard is here because that is a fact about the present, not a property of
 * the code: `RABBITMQ_QUEUE` is read from the environment, so the queue this
 * service declares is a deployment decision. Pointed at a queue another
 * service already declares, an argument here reproduces exactly the failure
 * that took the Worker off the bus:
 *
 ...
 *
 * RabbitMQ compares a redeclaration against the existing queue and rejects any
 * difference, then **closes the channel of whoever declared second**. When that
 * happened, every request stalled at RECEIVED while all six containers reported
 * healthy and every gate stayed green. Which service loses depends on boot
 * order, so the same code can work on one boot and fail on the next.
 *
